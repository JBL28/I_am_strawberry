from weather_api.weather_api_key import WEATHER_DECODING_API_KEY
import pandas as pd

import requests
from weather_api.utils import (get_current_date_string,
...
def get_grid_coordinates(location_list):
    # 엑셀 파일을 읽어 데이터프레임으로 변환
    file_path = "/Users/loyola/Dev/capstone/capstone-design/산학캡스톤/imStrawberry/weather_api/location_coordinate.xlsx"
    df = pd.read_excel(file_path)
    print(location_list)
    # 데이터 프레임 확인
    print(df.head())
    
    # location_list에서 1단계와 2단계를 추출
    stage1 = location_list[0]
    stage2 = location_list[1]
    
    
    # 1단계와 2단계에 맞는 행을 찾음
    row = df[(df['1단계'] == stage1) & (df['2단계'] == stage2)]
    
    print(f"Searching for: {stage1}, {stage2}")
...
def get_weather_forecast(location) -> str:
    ...
    """
    grid_x, grid_y = get_grid_coordinates(location)
    raw_weather_info, raw_special_report_info = get_weather_from_api(grid_x, grid_y)
...
def get_weather_from_api(nx, ny) -> list:
    """
    return: 
...
def filter_weather_info(raw_weather_info: list, raw_special_report_info: list, location: str) -> dict:
    ...
    for i in range(len(raw_weather_info)):
        ...
        if (weathercode in ["SKY", "PTY", "RN1", "WSD"] and
                (weathercode2text[weathercode] in filtered_weather_info and
    ...
    # 변화가 없으면, 1)현재 날씨와 2)5시간 뒤의 날씨 정보를 output
        ...
        after_5hrs_weather_info = dict((f"5시간 뒤 " + key, value) for key, value in after_5hrs_weather_info.items())
    ...
    for raw_special_report in raw_special_report_info:
        if location[1] in raw_special_report['t6']:
    ...
    if len(filtered_special_report_info) == 0:
        filtered_special_report_info["특보 발효 현황 내용"] = location[1] + "에 발효된 특보 및 주의보 없음"
    else:
